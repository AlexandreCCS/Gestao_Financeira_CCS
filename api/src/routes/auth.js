...
import { loginMega, getModulosUsuario } from '../soap/mega.js';
...
export default async function authRoutes(app) {
  ...
  app.post('/auth/login', {
    ...
    // [14/05/2026 - Alexandre Carvalho] Gate explicito: so 'A' (admin) ou 'U'
    // (usuario). Qualquer outro valor (inclusive 'N'/vazio) = sem acesso.
    if (!['A','U'].includes(u.PERM_GFIN)) {
    ...
    // [14/05/2026 - Alexandre Carvalho] Modulos visiveis: admin ve todos os
    // ativos, usuario so os liberados em CCS_TB_GFIN_PERM_USU.
    const modulos = await getModulosUsuario(u.GRU_IN_CODIGO, u.PERM_GFIN);
    ...
    const payload = {
      ...
      modulos,              // [{codigo,nome,ordem}] visiveis no menu
      jti
    };
  ...
  app.post('/auth/refresh', async (req, reply) => {
    ...
    sess.refresh = newRt;
    // [14/05/2026 - Alexandre Carvalho] Reconsulta modulos a cada refresh para
    // mudancas de permissao do admin propagarem sem exigir novo login.
    sess.modulos = await getModulosUsuario(Number(sess.sub), sess.perm);
    await setSession(app.redis, jti, sess);
    const accessToken = await reply.jwtSign({
      sub: sess.sub, login: sess.login, nome: sess.nome,
      email: sess.email, perm: sess.perm, modulos: sess.modulos, jti
    });
