...
// [23/07/2026 - Alexandre Carvalho] preHandler: bloqueia quem nao for admin
// (mesmo padrao do modulo de credito) - usado no PUT de parametros.
...
export default async function liberacaoBaixaRoutes(app) {
  ...
  // [23/07/2026 - Alexandre Carvalho] IMPCREC fora por padrao; parametro
  // LIB_BAIXA_MOSTRA_IMPCREC = 1 (modal Parametros) volta a exibir.
  // [23/07/2026 - Alexandre Carvalho] Empresas do grupo (agente com TAU 'G' no
  // GLO_AGENTES_ID: 100/200/300/400/401...) fora por padrao; parametro
  // LIB_BAIXA_MOSTRA_GRUPO = 1 volta a exibir.
  app.get('/liberacao-baixa/titulos', guard, async () => {
    const par = await megaQuery(`
      SELECT PARAM_ST_CHAVE CHAVE, PARAM_RE_VALOR V FROM MEGA.CCS_TB_GFIN_PARAM
       WHERE PARAM_ST_CHAVE IN ('LIB_BAIXA_MOSTRA_IMPCREC','LIB_BAIXA_MOSTRA_GRUPO')`);
    const pv = Object.fromEntries(par.map(r => [r.CHAVE, num(r.V)]));
    const filtroImpcrec = pv.LIB_BAIXA_MOSTRA_IMPCREC === 1 ? '' : `AND C.TPD_ST_CODIGO <> 'IMPCREC'`;
    // grupo = TAU 'G' no cadastro OU agente incluido manualmente na tabela de
    // extensao CCS_TB_GFIN_LIB_GRUPO_AGN (cadastros-cliente das empresas do grupo)
    const filtroGrupo = pv.LIB_BAIXA_MOSTRA_GRUPO === 1 ? '' : `
         AND NOT EXISTS (SELECT 1 FROM MEGA.GLO_AGENTES_ID GI
                          WHERE GI.AGN_IN_CODIGO = C.AGN_IN_CODIGO
                            AND GI.AGN_TAU_ST_CODIGO = 'G')
         AND NOT EXISTS (SELECT 1 FROM MEGA.CCS_TB_GFIN_LIB_GRUPO_AGN GX
                          WHERE GX.AGN_IN_CODIGO = C.AGN_IN_CODIGO)`;
  ...
  // [23/07/2026 - Alexandre Carvalho] Liberacao de Exc./Alt. — titulos BAIXADOS
  // (total ou parcial) SOMENTE das filiais da industria (varejo 401-404/501-504
  ...
  app.get('/liberacao-baixa/baixados', guard, async (req) => {
    ...
    const fim  = RX_DATA.test(req.query.fim || '') ? req.query.fim : hoje.toISOString().slice(0, 10);
    ...
    const rows = await megaQuery(`
      SELECT C.AGN_IN_CODIGO                                   AS AGN,
       ...
       WHERE (C.MOV_RE_VALOR - NVL(C.SALDO_EM_ABERTO, 0)) > 0.005
         ...
         AND C.TPD_ST_CODIGO <> 'PDV'
         ${filtroImpcrec}
         AND NVL(C.MOV_CH_SITUACAO,'A') <> 'C'
         AND TRUNC(C.MOV_DT_DATADOCTO) BETWEEN TO_DATE('${ini}','YYYY-MM-DD')
                                           AND TO_DATE('${fim}','YYYY-MM-DD')
         AND G.AGN_IN_CODIGO (+) = C.AGN_IN_CODIGO
       ORDER BY C.MOV_DT_DATADOCTO DESC, C.AGN_IN_CODIGO`);

    const titulos = rows.map(r => {
      const valor = num(r.VALOR), saldo = num(r.SALDO);
      return {
        agn_id:      num(r.AGN),
        cliente:     r.CLIENTE || `Cliente ${r.AGN}`,
        fil_id:      num(r.FIL),
        ...
        prorrogado:  r.PRORROGADO || null,
        valor,
        recebido:    Math.round((valor - saldo) * 100) / 100,
        saldo,
        baixa_total: saldo <= 0.005
      };
    });
    ...
    return {
      ...
      kpi: {
        qt_titulos:      titulos.length,
        vl_recebido:     titulos.reduce((s, t) => s + t.recebido, 0),
        qt_baixa_total:  tot.length,
        vl_baixa_total:  tot.reduce((s, t) => s + t.recebido, 0),
        qt_parcial:      parc.length,
        vl_parcial:      parc.reduce((s, t) => s + t.recebido, 0),
        saldo_parcial:   parc.reduce((s, t) => s + t.saldo, 0),
        qt_clientes:     new Set(titulos.map(t => t.agn_id)).size
      },
      titulos
    };
  ...
  // [23/07/2026 - Alexandre Carvalho] Liberacao de Exc./Alt. — libera alteracao/
  // exclusao de titulo baixado (trigger CCS_T_LIBERA_EXC_ALT em FIN_MOVIMENTO).
  app.post('/liberacao-exc/liberar', {
    ...guard,
    schema: {
      body: {
        type: 'object',
        required: ['fil', 'agn', 'documento'],
        properties: {
          fil:       { type: 'integer', minimum: 1 },
          agn:       { type: 'integer', minimum: 1 },
          documento: { type: 'string',  minLength: 1, maxLength: 20 },
    ...
    const { fil, agn } = req.body;
    const documento = lit(req.body.documento, 20);
    const parcela   = lit(req.body.parcela || '', 6);
    const motivo    = lit(req.body.motivo || '', 200);

    const dup = await megaQuery(`
      SELECT COUNT(*) N FROM MEGA.CCS_TB_LIBERA_EXC_ALT
       WHERE FIL_IN_CODIGO = ${num(fil)} AND AGN_IN_CODIGO = ${num(agn)}
         AND LIB_ST_DOCUMENTO = '${documento}'
         AND NVL(LIB_ST_PARCELA,'~') = NVL('${parcela}','~')
         AND LIB_CH_STATUS = 'A'`);
    if (num(dup[0]?.N) > 0) {
      return reply.code(409).send({ error: 'ja_liberado',
        message: 'Este título já tem uma liberação de exclusão/alteração ativa.' });
    }

    await megaExec(`
      BEGIN
        INSERT INTO MEGA.CCS_TB_LIBERA_EXC_ALT
          (LIB_IN_CODIGO, FIL_IN_CODIGO, AGN_IN_CODIGO, LIB_ST_DOCUMENTO, LIB_ST_PARCELA,
           LIB_ST_MOTIVO, LIB_CH_STATUS, LIB_IN_USU_LIBEROU, LIB_ST_USU_LIBEROU)
        VALUES
          (MEGA.CCS_SEQ_LIBERA_EXC_ALT.NEXTVAL, ${num(fil)}, ${num(agn)},
           '${documento}', ${parcela ? `'${parcela}'` : 'NULL'},
           ${motivo ? `'${motivo}'` : 'NULL'},
           'A', ${num(req.user.sub)}, '${lit(req.user.login, 60)}');
        COMMIT;
  ...
  app.post('/liberacao-exc/revogar', {
    ...guard,
    ...
    await megaExec(`
      BEGIN
        UPDATE MEGA.CCS_TB_LIBERA_EXC_ALT
           SET LIB_CH_STATUS = 'C',
  ...
  app.get('/liberacao-exc/liberacoes', guard, async (req) => {
    ...
    const [libs, execs] = await Promise.all([
      megaQuery(`
        SELECT L.LIB_IN_CODIGO ID, L.FIL_IN_CODIGO FIL, L.AGN_IN_CODIGO AGN,
          ...
          FROM MEGA.CCS_TB_LIBERA_EXC_ALT L,
               MEGA.GLO_AGENTES G,
      ...
      megaQuery(`
        SELECT E.LIB_IN_CODIGO ID, E.LIBE_CH_OPERACAO OPER, E.MOV_IN_NUMLANCTO LANC,
          ...
          FROM MEGA.CCS_TB_LIBERA_EXC_ALT_EXEC E,
    ...
    for (const e of execs) {
      ...
      porLib.get(k).push({
        operacao:      e.OPER,                    // U=alteração / D=exclusão
  ...
  // [23/07/2026 - Alexandre Carvalho] Parametros do modulo (CCS_TB_GFIN_PARAM).
  // GET aberto a autenticados (o modal exibe o valor); PUT restrito a admin.
  app.get('/liberacao-baixa/parametros', guard, async () => {
    const rows = await megaQuery(`
      SELECT PARAM_ST_CHAVE                                AS CHAVE,
             PARAM_RE_VALOR                                AS VALOR,
             TO_CHAR(PARAM_DT_ALTERACAO,'YYYY-MM-DD HH24:MI') AS ALTERADO_EM,
             PARAM_ST_USU                                  AS ALTERADO_POR
        FROM MEGA.CCS_TB_GFIN_PARAM
       WHERE PARAM_ST_CHAVE IN ('LIB_BAIXA_DIAS_UTEIS','LIB_BAIXA_MOSTRA_IMPCREC','LIB_BAIXA_MOSTRA_GRUPO')`);
    const por = Object.fromEntries(rows.map(r => [r.CHAVE, r]));
    const ult = rows
      .filter(r => r.ALTERADO_EM)
      .sort((a, b) => (b.ALTERADO_EM || '').localeCompare(a.ALTERADO_EM || ''))[0];
    return {
      dias_uteis_baixa: num(por.LIB_BAIXA_DIAS_UTEIS?.VALOR ?? 3),
      mostrar_impcrec:  num(por.LIB_BAIXA_MOSTRA_IMPCREC?.VALOR) === 1,
      mostrar_grupo:    num(por.LIB_BAIXA_MOSTRA_GRUPO?.VALOR) === 1,
      alterado_em:      ult?.ALTERADO_EM  || null,
      alterado_por:     ult?.ALTERADO_POR || null
    };
  });

  // [23/07/2026 - Alexandre Carvalho] Historico de alteracoes do parametro
  // (CCS_TB_GFIN_PARAM_HIST, alimentado por trigger a cada mudanca de valor).
  // Resolve o nome completo do usuario no GLO_GRUPO_USUARIO quando possivel.
  app.get('/liberacao-baixa/parametros/historico', guard, async () => {
    const rows = await megaQuery(`
      SELECT H.PARAM_ST_CHAVE                                 AS CHAVE,
             H.HIST_RE_VALOR_ANT                              AS DE,
             H.HIST_RE_VALOR_NOVO                             AS PARA,
             H.HIST_IN_USU                                    AS USU,
             NVL(NVL(U.GRU_ST_NOMECOMPLETO, U.GRU_ST_NOME), H.HIST_ST_USU) AS USUARIO,
             TO_CHAR(H.HIST_DT_ALTERACAO,'YYYY-MM-DD HH24:MI') AS QUANDO
        FROM MEGA.CCS_TB_GFIN_PARAM_HIST H,
             MEGA.GLO_GRUPO_USUARIO      U
       WHERE H.PARAM_ST_CHAVE IN ('LIB_BAIXA_DIAS_UTEIS','LIB_BAIXA_MOSTRA_IMPCREC','LIB_BAIXA_MOSTRA_GRUPO')
         AND U.GRU_IN_CODIGO (+) = H.HIST_IN_USU
       ORDER BY H.HIST_IN_CODIGO DESC
       FETCH FIRST 500 ROWS ONLY`);
    const bool = v => (num(v) === 1 ? 'Sim' : 'Não');
    const LABELS = {
      LIB_BAIXA_DIAS_UTEIS:     { rotulo: 'Dias úteis p/ baixa',      booleano: false },
      LIB_BAIXA_MOSTRA_IMPCREC: { rotulo: 'Mostrar IMPCREC',          booleano: true },
      LIB_BAIXA_MOSTRA_GRUPO:   { rotulo: 'Mostrar empresas do grupo', booleano: true }
    };
    return {
      historico: rows.map(r => {
        const cfg = LABELS[r.CHAVE] || { rotulo: r.CHAVE, booleano: false };
        return {
          parametro: cfg.rotulo,
          de:      r.DE === '' || r.DE == null ? null : (cfg.booleano ? bool(r.DE) : num(r.DE)),
          para:    cfg.booleano ? bool(r.PARA) : num(r.PARA),
          usuario: r.USUARIO || (r.USU ? `Usuário ${r.USU}` : '—'),
          quando:  r.QUANDO
        };
      })
    };
  ...
  app.put('/liberacao-baixa/parametros', {
    preHandler: [app.authenticate, adminOnly],
    schema: {
      body: {
        type: 'object', required: ['dias_uteis_baixa'],
        properties: {
          dias_uteis_baixa: { type: 'integer', minimum: 0, maximum: 365 },
          mostrar_impcrec:  { type: 'boolean' },
          mostrar_grupo:    { type: 'boolean' }
        }
      }
    }
  }, async (req) => {
    const dias = num(req.body.dias_uteis_baixa);
    // so grava valor DIFERENTE (a trigger de historico ja ignora igual, mas o
    // UPDATE cego carimbaria PARAM_DT_ALTERACAO/USU sem mudanca real)
    const sets = [
      { chave: 'LIB_BAIXA_DIAS_UTEIS', valor: dias },
      ...(typeof req.body.mostrar_impcrec === 'boolean'
        ? [{ chave: 'LIB_BAIXA_MOSTRA_IMPCREC', valor: req.body.mostrar_impcrec ? 1 : 0 }] : []),
      ...(typeof req.body.mostrar_grupo === 'boolean'
        ? [{ chave: 'LIB_BAIXA_MOSTRA_GRUPO', valor: req.body.mostrar_grupo ? 1 : 0 }] : [])
    ];
    const ups = sets.map(s => `
        UPDATE MEGA.CCS_TB_GFIN_PARAM
           SET PARAM_RE_VALOR     = ${s.valor},
               PARAM_DT_ALTERACAO = SYSDATE,
               PARAM_IN_USU       = ${num(req.user.sub)},
               PARAM_ST_USU       = '${lit(req.user.login, 60)}'
         WHERE PARAM_ST_CHAVE = '${s.chave}'
           AND PARAM_RE_VALOR <> ${s.valor};`).join('');
    await megaExec(`BEGIN${ups}\n        COMMIT;\n      END;`);
    return { ok: true, dias_uteis_baixa: dias,
             mostrar_impcrec: !!req.body.mostrar_impcrec,
             mostrar_grupo:   !!req.body.mostrar_grupo };
  });
